...
bool LoadTextureFromFile(const char* filename, MyTextureData* tex_data) {
  // Specifying 4 channels forces stb to load the image in RGBA which is an easy
  // format for Vulkan
  tex_data->Channels = 4;
  unsigned char* image_data = stbi_load(
      filename, &tex_data->Width, &tex_data->Height, 0, tex_data->Channels);
  ...
  // Create the Vulkan image.
  {
    ...
    check_vk_result(err);
    err =
        vkBindImageMemory(g_Device, tex_data->Image, tex_data->ImageMemory, 0);
    check_vk_result(err);
  }
  ...
  // Create Sampler
  {
    ...
    err = vkCreateSampler(g_Device, &sampler_info, g_Allocator,
                          &tex_data->Sampler);
    check_vk_result(err);
  }
  ...
  // Create Upload Buffer
  {
    ...
    err = vkCreateBuffer(g_Device, &buffer_info, g_Allocator,
    ...
    check_vk_result(err);
    err = vkBindBufferMemory(g_Device, tex_data->UploadBuffer,
  ...
  {
    void* map = NULL;
    err = vkMapMemory(g_Device, tex_data->UploadBufferMemory, 0, image_size, 0,
                      &map);
    check_vk_result(err);
  ...
  // Copy to Image
  {
    ...
    vkCmdCopyBufferToImage(command_buffer, tex_data->UploadBuffer,
                           tex_data->Image,
...
int main(int, char**) {
  ...
  io.Fonts->AddFontDefault();
  // io.Fonts->AddFontFromFileTTF("C:/Windows/Fonts/JetBrains Mono", 16.0f);
  // io.Fonts->AddFontFromFileTTF("../../misc/fonts/Cousine-Regular.ttf", 15.0f);
  // io.Fonts->AddFontFromFileTTF("../../misc/fonts/DroidSans.ttf", 16.0f);
  // io.Fonts->AddFontFromFileTTF("../../misc/fonts/ProggyTiny.ttf", 10.0f);
  const char* fontPath =
      "D:/cppproject/MyImguiDemo/src/resources/SmileySans-Oblique.ttf";
  ImFont* font = io.Fonts->AddFontFromFileTTF(
      fontPath, 25.0f, NULL, io.Fonts->GetGlyphRangesChineseFull());
  IM_ASSERT(font != NULL);
  ...
  while (!glfwWindowShouldClose(window)) {
    ...
    // to created a named window.
    {
        ...
        // some text (you can use a format strings too)
        ImGui::Checkbox("DemoWindow", &show_demo_window);      // Edit bools storing our window
        // open/close state
         ImGui::Checkbox("Another Window",
        &show_another_window);

        ImGui::SliderFloat("float", &f, 0.0f, 1.0f);            // Edit 1
        // float using a slider from 0.0f to 1.0f 
